...
function FormFarmManage() {
    ...
    const resolver: Resolver<FormCategoryValue> = async (values) => {
        return {
    ...
    const { resultCate, loadingCate, errorCate } = useAppSelector((state: any) => state.category)
    // const { resultPrds, loadingPrds, errorPrds } = useAppSelector((state: any) => state.category)
    const { register, handleSubmit, reset, formState: { errors } } = useForm<FormCategoryValue>({ resolver })
    ...
    const onSubmitCate = ((data: CateType) => {
    ...
    const onSubmitPrds = ((data: CateType) => {
        dispatch(apiAddCate(data))
            .unwrap().then(() => {
