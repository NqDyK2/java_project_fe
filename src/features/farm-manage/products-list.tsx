...
import { apiGetAllProductsOfUser } from '../detail-seller/seller.action';
...
const ProductsList = (props: Props) => {
    ...
    const allProducts = useAppSelector((state: any) => state.seller)
    ...
    useEffect(() => {
        ...
        dispatch(apiGetAllProductsOfUser({ id: getIdLocal.id, page: page, size: 6 }))
    }, [dispatch, page])

    const list = allProducts?.result?.result
                            ...
                            <table className="min-w-full bg-white dark:bg-gray-800">
                                <thead>
                                    <tr className="w-full h-16 border-gray-300 dark:border-gray-200 border-b py-8">
                                        ...
                                        <th className="text-gray-600 dark:text-gray-400 font-normal pr-6 text-left text-sm tracking-normal leading-4">Tên sản phẩm</th>
                                ...
                                <tbody>
                                    {
                                        productsWithCategories?.map((item: any) => (
                                            <tr key={item.id} className="h-24 border-gray-300 dark:border-gray-200 border-b">
                                                ...
                                                <td className="text-sm pr-6 whitespace-no-wrap text-gray-800 dark:text-gray-100 tracking-normal leading-4 text-left">{item.title ? item.title : "Thiếu thông tin"}</td>
                                                <td className="text-sm pr-6 whitespace-no-wrap text-gray-800 dark:text-gray-100 tracking-normal leading-4 text-left"><img src={item.image} alt="" width={100} /></td>
